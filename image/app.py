...
def overlay_frame(result_img, frame_path, person_img, product_img):

    rotation_angle = 9

    width = height = 135

    rectangle_coords = (17, 310, width, height)
    ...
    # Calculate the size of the rectangle where the person_img should be placed
    rect_x, rect_y, rect_width, rect_height = rectangle_coords

    # Resize the person_img to fit inside the rectangle
    person_resized = cv2.resize(person_img, (rect_width, rect_height))

    # Get the center of the person image
    center = (person_resized.shape[1] // 2, person_resized.shape[0] // 2)

    # Calculate the rotation matrix
    rotation_matrix = cv2.getRotationMatrix2D(center, rotation_angle, 1)

    # Apply the rotation matrix to the person image
    person_rotated = cv2.warpAffine(
        person_resized, rotation_matrix, (person_resized.shape[1], person_resized.shape[0]))

    # Place the rotated person_img on the result_img at the specified rectangle coordinates
    result_img[rect_y:rect_y + rect_height,
               rect_x:rect_x + rect_width] = person_rotated
    ...
    # Resize the product image to a smaller size
    # Adjust the size (30, 30) as needed
    product_resized = cv2.resize(product_img, (40, 40))

    # Choose the position where you want to place the product image on the frame image
    product_x, product_y = 150, 65  # Adjust the position as needed

    # Overlay the resized product image on the frame image
    if product_resized.shape[2] == 4:
        product_alpha = product_resized[:, :, 3] / 255.0
        product_alpha = np.expand_dims(product_alpha, axis=2)

        result_img[product_y:product_y + product_resized.shape[0],
                   product_x:product_x + product_resized.shape[1]] = (1 - product_alpha) * result_img[product_y:product_y + product_resized.shape[0], product_x:product_x + product_resized.shape[1]] + product_alpha * product_resized[:, :, :3]
    else:
        result_img[product_y:product_y + product_resized.shape[0],
                   product_x:product_x + product_resized.shape[1]] = product_resized
...
def overlay():
    ...
    result_with_frame = overlay_frame(
        result, frame_path, person_copy, product_img)
